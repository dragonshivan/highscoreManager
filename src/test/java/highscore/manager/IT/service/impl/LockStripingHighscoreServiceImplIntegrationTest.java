...
public class LockStripingHighscoreServiceImplIntegrationTest {
	...
	public void WHEN_update_is_ongoing_THEN_read_on_same_level_waits_for_it_to_finish() {
		//when
		Supplier<HighscoreService> context = () -> {
			HighscoreService highscoreService = new LockStripingHighscoreServiceImpl(new SlowWriteCappedIntHashMapFactory((byte)15), 10);
	...
	public void WHEN_read_is_ongoing_THEN_update_on_same_level_waits_for_it_to_finish() {
		//when
		Supplier<HighscoreService> context = () -> {
			HighscoreService highscoreService = new LockStripingHighscoreServiceImpl(new SlowStreamCappedIntHashMapFactory((byte)15), 10);
			highscoreService.update(1, 11, 111);
			return highscoreService;
		};
		ActorAction<HighscoreService, IntHeap> read = (highscoreService) -> {
